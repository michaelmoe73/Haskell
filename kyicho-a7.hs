...
evalBlock env (Block ids rhss cmdexps) =
  do vs <- evalExps env rhss
     rs <- newRefs vs
     let env' = extendEnv ids rs env {- ExtendedEnv (Frame ["r","n"] [##,##]) EmptyEnv -}
...
evalCmd env (For id elo ehi eby block) = 
 do vs <- evalExp env elo
    rs <- newRef vs
    let env' = extendEnv [id] [rs] env
    forloop env' (Int 0)
    where forloop env' r = do (Int x) <- readRef (applyEnv env' "i")
                              b <- evalExp env' (PrimitiveApp LessOrEq [Number x, ehi])
                              case b of
                                Bool True -> do r' <- evalBlock env' block
                                                m <- evalExp env' (PrimitiveApp Add [Number x, (if eby == Null then (Number 1) else eby)])     
                                                writeRef (applyEnv env' "i") m
                                                forloop env' r'
                                other -> return r
  
evalCmd env (Flet id e b) = 
  do (Int x) <- readRef (applyEnv env id)
     ex <- evalExp emptyEnv e
     writeRef (applyEnv env "x") ex
     y <- evalBlock env b
     mx <- evalExp emptyEnv (Number x)
     writeRef (applyEnv env "x") mx
     return y 


evalCmd env (Setfst epair e) =
  setfst env epair
  where setfst env epair = do (PairVal a b) <- evalExp env epair
                              t <- evalExp emptyEnv e
                              writeRef a t
                              x <- evalExp env epair
                              return x;
...
evalCmd env (Setsnd epair e) =
  setsnd env epair
  where setsnd env epair = do (PairVal a b) <- evalExp env epair
                              t <- evalExp env e
                              writeRef b t
                              x <- evalExp env e
                              return x;
...
applyPrimitive Fst [PairVal u v] = do {x <- readRef u; return x} {-undefined-}
applyPrimitive Snd [PairVal u v] = do {x <- readRef v; return x} {-undefined-}
applyPrimitive IsNull [v] = 
  case v of
   NullVal -> return $ Bool True
   other -> return $ Bool False {-undefined-}
